...
type Bday = { id: string; name: string; category: string | null; date: string; notes: string | null; gift_ideas: string | null };

const empty = () => ({ name: "", category: "amigos", date: "", notes: "", gift_ideas: "" });
...
function AniversariosPage() {
  ...
  useEffect(() => {
    if (!open) return;
    setForm(editing ? {
      name: editing.name, category: editing.category ?? "amigos", date: editing.date,
      notes: editing.notes ?? "", gift_ideas: editing.gift_ideas ?? "",
    } : empty());
  }, [open, editing]);

  const { data: list } = useQuery({
    enabled: !!user, queryKey: ["birthdays", user?.id],
    queryFn: async () => ((await supabase.from("birthdays").select("*")).data ?? []) as Bday[],
  });

  const save = async () => {
    if (!user || !form.name || !form.date) return;
    const { error } = editing
      ? await supabase.from("birthdays").update(form).eq("id", editing.id)
      : await supabase.from("birthdays").insert({ ...form, user_id: user.id });
  ...
  const remove = async () => {
    if (!confirmId) return;
    await supabase.from("birthdays").delete().eq("id", confirmId);
    setConfirmId(null);
    qc.invalidateQueries({ queryKey: ["birthdays"] });
  };
  ...
  return (
    <div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle className="font-display">{editing ? "Editar aniversário" : "Novo aniversário"}</DialogTitle></DialogHeader>
      ...
      <ConfirmDialog open={!!confirmId} onOpenChange={(o) => !o && setConfirmId(null)} onConfirm={remove} title="Excluir aniversário?" />
    </div>
  );
}
