...
import { Calendar as CalIcon, Plus, ChevronLeft, ChevronRight, Trash2, Check } from "lucide-react";
...
function AgendaPage() {
  ...
  const [form, setForm] = useState({
    title: "", date: format(new Date(), "yyyy-MM-dd"), time_str: "", type: "evento", description: "",
  });

  const { data: events } = useQuery({
    enabled: !!user, queryKey: ["events", user?.id],
    queryFn: async () => (await supabase.from("events").select("*").order("date")).data ?? [],
  });
  ...
  const save = async () => {
    ...
    setOpen(false); qc.invalidateQueries({ queryKey: ["events"] }); qc.invalidateQueries({ queryKey: ["dashboard"] });
  };
  const remove = async (id: string) => { await supabase.from("events").delete().eq("id", id); qc.invalidateQueries({ queryKey: ["events"] }); };
          ...
          <div className="space-y-2">
            {(selected ? selectedEvents : (events ?? [])).slice(0, 20).map((e) => (
              <div key={e.id} className="cozy-card flex items-center gap-3 p-3">
                ...
                <button onClick={() => remove(e.id)} className="grid h-8 w-8 place-items-center rounded-lg text-muted-foreground hover:text-destructive"><Trash2 className="h-4 w-4" /></button>
              </div>
            ))}
          </div>
        )}
